...
export default function GetStartedPage() {
  ...
  const handleNext = async () => {
    ...
    if (step === 2) {
      if (selectedPlayers.length > 0) {
        try {
          setLoading(true);
          await saveFavorites({ favoritePlayers: selectedPlayers.map((p) => Number(p.id)) });
    ...
    if (selectedTeams.length > 0) {
      try {
        setLoading(true);
        await saveFavorites({ favoriteTeams: selectedTeams.map((t) => Number(t.id)) });
  ...
  const addPlayer    = (p: SelectedItem) => setSelectedPlayers((prev) => [...prev, p]);
  ...
  const removeTeam = (id: string)      => setSelectedTeams((prev) => prev.filter((t) => t.id !== id));
